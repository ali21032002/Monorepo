...
app = FastAPI(
    ...
    version="0.2.0",
    description="سرویس استخراج و تحلیل متن با پشتیبانی از چندین مدل و قابلیت‌های پیشرفته",
    docs_url="/docs",
    redoc_url="/redoc",
...
@app.get("/api/health",
         ...
         description="بررسی وضعیت کلی سرویس و اتصال به Ollama",
         tags=["System"])
...
@app.get("/api/schemas", 
         ...
         summary="لیست اسکیماهای موجود",
         description="دریافت لیست تمام اسکیماهای استخراج موجود",
         tags=["Schemas"])
...
@app.get("/api/domains", 
         ...
         summary="لیست دامنه‌های موجود",
         description="دریافت لیست دامنه‌های تحلیل موجود",
         tags=["Domains"])
...
@app.get("/api/speech-models",
         summary="مدل‌های گفتار به متن",
         description="دریافت اطلاعات مدل‌های گفتار به متن موجود",
         tags=["Speech Models"])
...
@app.get("/api/config/tokens",
         summary="تنظیمات توکن",
         description="دریافت تنظیمات فعلی توکن‌ها",
         tags=["Configuration"])
...
@app.post("/api/extract", 
           ...
           summary="استخراج موجودیت‌ها و روابط",
           description="استخراج موجودیت‌ها و روابط از متن با استفاده از مدل‌های هوش مصنوعی",
           tags=["Extraction"])
...
@app.post("/api/extract_file", 
           response_model=ExtractionResponse,
           summary="استخراج از فایل",
           description="استخراج موجودیت‌ها و روابط از فایل آپلود شده",
           tags=["Extraction"])
async def extract_file(
    file: UploadFile = File(..., description="فایل برای استخراج"),
    language: str = Form("fa", description="زبان متن"),
    schema_name: str = Form("general", alias="schema", description="نام اسکیما"),
    model: Optional[str] = Form(None, description="مدل مورد استفاده"),
    temperature: Optional[float] = Form(None, description="درجه حرارت مدل"),
    max_output_tokens: Optional[int] = Form(None, description="حداکثر تعداد توکن خروجی"),
) -> ExtractionResponse:
...
@app.post("/api/multi_extract", 
           response_model=MultiModelResponse,
           summary="استخراج چندمدله",
           description="استخراج موجودیت‌ها و روابط با استفاده از چندین مدل و داوری",
           tags=["Multi-Model"])
...
@app.post("/api/chat", 
           ...
           summary="چت هوشمند",
           description="چت تعاملی با پشتیبانی از سابقه مکالمه و تحلیل",
           tags=["Chat"])
...
@app.post("/api/speech-to-text", 
           ...
           description="تبدیل فایل صوتی به متن با استفاده از سرویس گفتار به متن",
           tags=["Speech-to-Text"])
async def speech_to_text(
    audio_file: UploadFile = File(..., description="فایل صوتی برای تبدیل"),
    language: str = Form("fa", description="زبان گفتار"),
    use_hybrid: bool = Form(True, description="استفاده از مدل ترکیبی"),
    model_preference: str = Form("auto", alias="model_preference", description="ترجیح مدل")
) -> SpeechToTextResponse:
